...
var openBasketButton = document.getElementByTd( 'open-basket-button');
...
var computerModalDescription = document.getElementByld('computer-modal-description');
...
var computerModalDrive = document.getElementByTd('computer-modal-drive')
var computerModalDriveType = document.getElementById(' computer-modal-drive-type');
var computerModal0s = document.getElementById( 'computer-modal-os')
...
var basketComputersTableBodyElement = document.getElementByld('basket-computers-table-body') ;
var basketTotalPriceContentElement = document.getElementById('basket-total-price-content');

var users = []
var categories = []
var categoriesGlobal = []
var computers = []; var computersGlobal = []; var computerSelectedGlobal  = []
var computerSelectedGlobalForSearch = []

var basketComputers = []
var currentSelectedCategoryId = 0;

function loadDataFromLocalStrorage() {
...
function loadComputerCategories() {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        "list-group-item-action' id='computer-category-" + c.id + "'onclick='onCategorySelected(" + c.id +"'onclick='onCategorySelected(" +c.id + ")'>" + c.name + "</li>"
    }
...
function onCategorySelected(categoryId) {
    if (currentSelectedCategoryId === categoryId) { } else{
        ...
        setTimeout(function() {
            ...
            computerSelectedGlobalForSearch = computersSelected.slice()
...
function onComputerSelected(computerId) {
    ...
    computerModalPhone.innerHTML = selectedComputer.ohine;
    ...
    computerModalDriveType.innerHTML = selectedComputer.DriveType == "hdd" ? "HDD" : "SDD"
    computerModal0s.innerHTML = selectedComputer.os
...
function saveBasketComputersToLocalStorage() {
    localStorage.setItem('basketVomputers', JSON.stringify(basketComputers))
}
...
function onOpenBasket(params) {
    if (basketComputer.length === 0) {
        showAlertMessage('Корзина пуста', 1000)
    } else {
        basketModalElement.style.display = "block"
        refreshComputerBasket()
...
function refreshComputerBasket() {
    ...
    for (let index = 0; index < basketComputers.length; index++) {
        ...
        basketComputersTableBodyElementHtml += '</td><td><img class="basket-computer-image" src="' + b.computer.imagepath + '">' 
        basketComputersTableBodyElementHtml += '<td></td>' + b.computer.name
        basketComputersTableBodyElementHtml += '</td><td>' + b.computer.price
        basketComputersTableBodyElementHtml += ' AZN></td><td><input min="1" max="10000" type="number" value="' + b.count + '" ' + ' onchange="computerCountChanged(this,' +b.computer.id + ')" onkeypress="checkCount(event)" />'
        basketComputersTableBodyElementHtml += '</td><td id="computer-total-price-' +  b.computer.id + '">'+ (b.computer.price * b.count)
        basketComputersTableBodyElementHtml += ' AZN</td><td><button onclick="deleteBasketComputer(' + b.computer.id + ')" class="btn btn-danger">Удалить</button></td><tr>'
...
function onOrderSubmit(event) {
    ...
    orderObject.basketComputers = basketComputers.value
    var customer = {}
    ...
    for (let index = 0; index < userIdList.length; index++) {
        ...
        for (let i = 0; i < orderObject.basketComputers.length; i++) {
            ...
            if (b.computer.userId === userIdList[index]) {
                orderObject.basketComputers.push(b)
            }
        }
...
endOrderRegistraiton(customer)
}

function endOrderRegistraiton(customer) {
    basketComputers.splice(0, basketComputers.length);
...
function calculateOrdertTotalPrice(order) {
    var totalPrice = 0;
...
setTimeout(function() {
    ...
    computerSelectedGlobal = computers.slice()

    for (let index = 0; index < computerSelectedGlobal.length; index++) {
        const c = computerSelectedGlobal[index];
    ...
    computerSelectedGlobal = findedComputers.slice()
    findedComputers = findedComputers.slice(begin, length)
    console.log(findedComputers.length)
    addComputersToPage(findedComputers);
    computersElement.style.display = 'block';
    if (findedComputers.length == 0) {
        computersElement.innerHTML = "<h2 class='not-found'>Не найдено компьютеров по вашему запросу!</h2>";
    }
    allComputersLoaded = false
}, 500);
    }
...
let computerModalImageContainer = document.getElementById('computer-modal-image-containter')
...
let computerLoadingNext = document.getElementById("computer-loading-next")
    ...
    if (allComputersLoaded) {
        ...
        if (allowScroll) {
            ...
            if (distanceToBottom < clientHeight + 150) {
                ...
                setTimeout(function() {
                    if (computerSelectedGlobal.length <= (begin + length)) {
                        allComputersLoaded = true; computersLoadingNext.style.display = 'none'
                    } else{
                        begin += length
                        let nextComputers = computerSelectedGlobal.slice(begin, begin + length)
